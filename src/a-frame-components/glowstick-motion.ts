...
// A-Frame exposes three.js as a global (set by aframe.min.js).
...
// in lockstep. Not exposed — the brief only asks for a single idle *radius* knob.
const IDLE_SPEED = 1.0;

// Per-instance animation for one glowstick, authored by glowstick-field onto each
// `.lightstick-instance`. Two motions, composed onto the transform every frame:
//
//   Wave  — a "someone is waving it" forward/back swing that fades in as the camera
//           approaches (waveNear/waveFar, smoothstep). It swings along the azimuth
//           the stick already leans toward (from its base X/Z tilt), so it nods
//           forward and back rather than side to side. sin() drives it, so it eases
//           naturally to a stop at each far end of the swing.
//   Idle  — a subtle, always-on float in every direction (independent per-axis
//           phases/frequencies), bounded by idleRadius.
//
// It captures the base position/rotation set by glowstick-field once, then writes
// base + wave + idle straight to object3D each tick (cheap; no attribute reparsing).
export default {
  schema: {
    // Wave fades in across this distance band (metres, camera → stick).
    ...
    // Y offset of the wave pivot along the stick's own axis, in local units.
    // 0 = pivot at the stick origin; negative lowers it (e.g. toward the "hand"),
    // positive raises it. Only affects the wave, not the idle float.
  ...
  init() {
    ...
    // Base transform authored by glowstick-field, captured before we animate it.
    ...
    // Wave azimuth from the base lean. rot.x leans the stick along Z, rot.z along X;
    // adding the swing proportionally to this unit vector oscillates the lean
    // magnitude along one fixed azimuth (a forward/back nod). An upright stick has
    ...
    // Random per-stick phases so the field doesn't wave/float in unison.
    ...
    // Wave pivot: a point pivotY (scaled by the instance scale) along the stick's
