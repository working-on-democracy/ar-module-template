...
export default {
  ...
  init() {
    ...
    // A descendant lod-object may have already registered before this
    // component's own init() ran (see register()) — don't clobber it.
  ...
  register(obj: any) {
    // A child lod-object's init() can run before this component's own init()
    // has set up `objects` (A-Frame doesn't guarantee ancestor-before-
    // descendant component init order), so guard rather than assume it exists.
    const self = this as any;
