// @ts-nocheck
// ---------------------------------------------------------------------------
// DMS "Liquid Civic Mirror" A-Frame components, ported from the old standalone
// 8th Wall + webpack template (DROP_INTO_AR_HFG_TEMPLATE/src/integrations/dms/
// mirror-shards.js) into this Vue/manifest ArModule template.
//
// What changed vs. the original:
//   - Runtime `fetch()` of the JSON data files is replaced by static `import`s
//     (bundled into the module) so nothing has to be served at fixed URLs.
//   - Texture URLs are resolved from bundled `import`s (Vite rewrites them to a
//     URL relative to the emitted ar-module.js via import.meta.url), so the
//     poster images resolve wherever the module is hosted.
//   - The A-Frame component *definitions* are exposed to the manifest via
//     getDmsComponents() (see bottom of file) instead of self-registering; the
//     host registers them from manifest.components before mounting ArModule.vue.
//   - The DOM chrome (control bar, status text) and its wiring live in
//     ArModule.vue, which calls the exported injectDmsStyles() / wireDmsControls()
//     / setupDmsWorldLifecycle() helpers on mount.
//
// The bulk of the rendering/interaction logic below is unchanged from the
// original; @ts-nocheck keeps this large ported vanilla-JS module out of strict
// type-checking (matching the loose `this: any` style of the other components).
// ---------------------------------------------------------------------------
...
const DMS_TEXTURE_URLS = {
  'single-liquid-final-target.png': finalTargetUrl,
  'authoritarian-orange-final-target-v1.png': authorityTargetUrl,
}
...
function normalizeDmsPath(src) {
  if (!src) return src
  // Resolve poster images to the URLs Vite bundled for them (keyed by file name).
  const fileName = String(src).split('/').pop()
  if (fileName && DMS_TEXTURE_URLS[fileName]) return DMS_TEXTURE_URLS[fileName]
...
function loadVariantData(defaultData) {
  // Variant data is bundled (was: fetched from SHARD_VARIANT_URLS at runtime).
  const map = new Map()
  if (shardsImpactStarData) map.set('impact_star', shardsImpactStarData)
...
function getCameraObject() {
  // The old standalone scene owned <a-camera id="camera">. In this template the
  // host provides the camera and it may not carry that id, so fall back to any
  // a-camera / [camera] entity, then to the scene's active THREE camera.
  const cameraEl = document.querySelector('#camera')
    || document.querySelector('a-camera')
    || document.querySelector('[camera]')
  const fromEl = cameraEl?.getObject3D?.('camera') || cameraEl?.object3DMap?.camera || cameraEl?.object3D
  ...
  window.AFRAME.registerComponent('dms-mirror-shards', {
    ...
    async load() {
      try {
        ...
        // Data is bundled with the module (was: fetched from this.data.dataUrl /
        // this.data.seriesUrl). The schema still carries those fields for compat.
        const data = shardsData
        const rawSeries = seriesData
        const series = normalizeSeries(rawSeries)
        const variantData = loadVariantData(data)
    ...
    onPlacementPointerDown(event) {
      ...
      if (event.target?.closest?.('.dms-control-bar')) return
...
// ---------------------------------------------------------------------------
// Manifest integration for the ArModule template.
//
// The host registers A-Frame components from `manifest.components` (a name →
// definition map) *before* mounting the module. The original code registered its
// components directly via window.AFRAME.registerComponent inside registerDms*()
// wrappers. getDmsComponents() reuses those wrappers unchanged: it temporarily
// swaps in a capturing stub for window.AFRAME so each registerComponent(name,
// def) call records the definition instead of registering it, then restores the
// real AFRAME. The captured map is handed to the manifest, and the host does the
// real registration. Works whether or not AFRAME is loaded yet at import time.
// ---------------------------------------------------------------------------
export function getDmsComponents(): Record<string, any> {
  const captured = {}
  const previousAframe = window.AFRAME
  window.AFRAME = {
    components: {},
  ...
  try {
    // registerDmsMirrorShards() also registers the diagnostic/anchor-test helper
    // components; they're inert unless attached to an entity, so capturing them
    // is harmless and keeps the original wiring intact.
    registerDmsMirrorShards()
  } finally {
    window.AFRAME = previousAframe
  }
  return captured
}

// The prototype's DOM chrome (control bar + status text) styling. Injected once
// at runtime rather than shipped as a separate .css file, so the module stays a
// single self-contained ES bundle. (Originally src/index.css; scene/body-level
// resets were dropped so the module doesn't clobber the host page.)
const DMS_CSS = `
:root {
  --dms-panel: rgba(7, 10, 12, 0.66);
  --dms-panel-edge: rgba(232, 255, 255, 0.2);
  --dms-ink: rgba(244, 252, 252, 0.94);
}
.dms-runtime-warning {
  position: fixed;
  left: 50%;
  ...
  border: 1px solid var(--dms-panel-edge);
  border-radius: 8px;
...
.dms-target-status {
  ...
  padding: 10px 14px;
  color: rgba(244, 252, 252, 0.94);
  background: rgba(8, 12, 14, 0.7);
  border: 1px solid rgba(232, 255, 255, 0.18);
  border-radius: 999px;
...
.dms-target-status[hidden] {
  display: none;
}
.dms-target-status.is-debug {
...
.dms-icon-button.is-blue {
  color: rgba(230, 244, 255, 0.96);
  background: rgba(21, 89, 255, 0.18);
  border-color: rgba(100, 165, 255, 0.42);
}
.dms-icon-button.is-orange {
  color: rgba(255, 238, 224, 0.96);
  background: rgba(228, 87, 24, 0.20);
  border-color: rgba(255, 154, 88, 0.46);
}
.dms-icon-button:active {
  transform: scale(0.94);
}
.dms-icon-button:disabled,
.dms-icon-button.is-cooling {
  cursor: wait;
  opacity: 0.48;
  filter: saturate(0.78) brightness(0.92);
}
.dms-icon-button.is-cooling::after {
  content: "";
  position: absolute;
  inset: 5px;
...
/** Inject the DMS UI styles once (idempotent). Called from ArModule.vue's mount. */
export function injectDmsStyles() {
  if (typeof document === 'undefined') return
  if (document.getElementById('dms-mirror-shards-styles')) return
  const style = document.createElement('style')
  style.id = 'dms-mirror-shards-styles'
  style.textContent = DMS_CSS
